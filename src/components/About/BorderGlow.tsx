...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    const c = colors[Math.min(COLOR_MAP[i] ?? 0, colors.length - 1)] ?? "#c084fc";
...
export const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    <div
      ...
      className={`relative grid isolate ${isTransparent ? "" : "border"} ${className}`}
      style={{
        background: backgroundColor,
        borderColor: isTransparent ? "transparent" : lightSurface ? "rgb(24 24 27 / 12%)" : "rgb(255 255 255 / 15%)",
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          background: [
            ...(isTransparent ? [] : [`linear-gradient(${backgroundColor} 0 100%) padding-box`]),
      ...
      <div className={`relative z-[1] ${containerClassName}`}>{children}</div>
    </div>
  );
};
