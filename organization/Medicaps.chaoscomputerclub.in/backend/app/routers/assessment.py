...
async def get_or_create_assessment(slug: str, db: AsyncSession) -> Assessment:
    """Find assessment by slug, or dynamically provision one from matching OfflineContest or default."""
    result = await db.execute(select(Assessment).where(Assessment.slug == slug))
    assessment = result.scalars().first()
    if assessment:
        return assessment

    # Fallback to matching OfflineContest
    c_res = await db.execute(select(OfflineContest).where(OfflineContest.slug == slug))
    contest = c_res.scalars().first()

    title = f"{contest.title} — Online Screening" if contest else f"Assessment Round: {slug}"
    summary = contest.summary if contest else "Online screening assessment round for Chaos Computer Club."
